...
NSString * const QQM_TABLE_NAME = @"QQM_TABLE_NAME";

@interface QQMDataManager ()
/**
 *  <#describle#>
 */
...
@implementation QQMDataManager

+ (QQMDataManager *)managerWithTable:(NSString *)modelName {
    return [[QQMDataManager alloc] initWithTable:modelName];
}
...
- (NSArray<BaseModel *> * )fetch:(NSDictionary *)dic {
    if ([self.db open]) {
        ...
        NSMutableString * sql = [NSMutableString stringWithFormat:@"SELECT * FROM %@ where ",dic[QQM_TABLE_NAME]];
        
        for (int i = 0; i < dic.allKeys.count; i++) {
            NSString * member = dic.allKeys[i];
            if (![member isEqualToString:QQM_TABLE_NAME]) {
        ...
        while ([rs next]) {
            ...
            NSDictionary * dataDic = [self returnDicWithClassName:dic[QQM_TABLE_NAME]];
            [arr addObject:[self setUpModelWithDic:dataDic andResult:rs withClassName:dic[QQM_TABLE_NAME]]];
        }
        [self.db close];
        return arr;
    } else {
...
- (id)returnContent:(BaseModel *)model withKey:(NSString *)key{
...
- (FMDatabase *)db {
    if (!_db) {
        ...
        NSString * dbpath = [docsdir stringByAppendingPathComponent:@"qqm_data.sqlite"];
        NSLog(@"%@",dbpath);
