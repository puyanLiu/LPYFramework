...
- (void)setArrow:(BOOL)arrow
{
    ...
    // 遍历子控件，改变其是否有箭头
    [self.subviews enumerateObjectsUsingBlock:^(LPYCircle *circle, NSUInteger idx, BOOL *stop) {
...
-(void)lockViewPrepare{
    ...
    for (NSUInteger i=0; i<9; i++) {
        ...
        circle.type = LPYCircleTypeGesture;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(LPYCircle *circle, NSUInteger idx, BOOL *stop) {
        if (CGRectContainsPoint(circle.frame, point)) {
            [circle setState:LPYCircleStateSelected];
            [self.circleSet addObject:circle];
        }
    }];
    
    // 数组中最后一个对象的处理
    [self circleSetLastObjectWithState:LPYCircleStateLastOneSelected];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.circleSet enumerateObjectsUsingBlock:^(LPYCircle *circle, NSUInteger idx, BOOL *stop) {
        
        [circle setState:LPYCircleStateSelected];
        
        // 如果是登录或者验证原手势密码，就改为对应的状态
        if (self.type != LPYCircleViewTypeSetting) {
            [circle setState:LPYCircleStateLastOneSelected];
        }
    }];
...
- (void)errorToDisplay
{
    if ([self getCircleState] == LPYCircleStateError || [self getCircleState] == LPYCircleStateLastOneError) {
...
- (void)gestureEndResetMembers
{
    @synchronized(self) { // 保证线程安全
        if (!self.hasClean) {
            ...
            // 手势完毕，选中的圆回归普通状态
            [self changeCircleInCircleSetWithState:LPYCircleStateNormal];
...
- (LPYCircleState)getCircleState
{
    return [(LPYCircle *)[self.circleSet firstObject] state];
}

#pragma mark - 清空所有子控件的方向
- (void)resetAllCirclesDirect
{
    [self.subviews enumerateObjectsUsingBlock:^(LPYCircle *obj, NSUInteger idx, BOOL *stop) {
        [obj setAngle:0];
    }];
}

#pragma mark - 对数组中最后一个对象的处理
- (void)circleSetLastObjectWithState:(LPYCircleState)state
{
...
- (void)gestureEndByTypeSettingWithGesture:(NSString *)gesture length:(CGFloat)length
{
    if (length < CircleSetCountLeast) {
        ...
        // 2.改变状态为error
        [self changeCircleInCircleSetWithState:LPYCircleStateError];
        ...
        if ([gestureOne length] < CircleSetCountLeast) { // 接收并存储第一个密码
            // 记录第一次密码
            [LPYCircleViewConst saveGesture:gesture key:gestureOneSaveKey];
            ...
            BOOL equal = [gesture isEqual:[LPYCircleViewConst getGestureWithKey:gestureOneSaveKey]]; // 匹配两次手势
...
{
    NSString *password = [LPYCircleViewConst getGestureWithKey:gestureFinalSaveKey];
...
- (void)changeCircleInCircleSetWithState:(LPYCircleState)state
{
    [self.circleSet enumerateObjectsUsingBlock:^(LPYCircle *circle, NSUInteger idx, BOOL *stop) {
        ...
        if (state == LPYCircleStateError) {
            if (idx == self.circleSet.count - 1) {
                [circle setState:LPYCircleStateLastOneError];
            }
        }
...
- (NSString *)getGestureResultFromCircleSet:(NSMutableArray *)circleSet
{
    ...
    for (LPYCircle *circle in circleSet) {
        // 遍历取tag拼字符串
...
- (void)drawRect:(CGRect)rect
{
    ...
    UIColor *color;
    if ([self getCircleState] == LPYCircleStateError) {
...
- (void)connectCirclesInRect:(CGRect)rect lineColor:(UIColor *)color
{
    ...
    for (int index = 0; index < self.circleSet.count; index++) {
        ...
        // 取出选中按钮
        LPYCircle *circle = self.circleSet[index];
...
-(void)calAngleAndconnectTheJumpedCircle{
    ...
    //取出最后一个对象
    LPYCircle *lastOne = [self.circleSet lastObject];
    
    //倒数第二个
    LPYCircle *lastTwo = [self.circleSet objectAtIndex:(self.circleSet.count -2)];
    ...
    LPYCircle *centerCircle = [self enumCircleSetToFindWhichSubviewContainTheCenterPoint:center];
 ...
 *  @return 点所在的圆
 */
- (LPYCircle *)enumCircleSetToFindWhichSubviewContainTheCenterPoint:(CGPoint)point
{
    LPYCircle *centerCircle;
    for (LPYCircle *circle in self.subviews) {
        if (CGRectContainsPoint(circle.frame, point)) {
            centerCircle = circle;
        }
    }
