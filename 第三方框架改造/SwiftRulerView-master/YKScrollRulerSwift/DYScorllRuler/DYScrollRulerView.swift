...
fileprivate let RulerGap         = 10
fileprivate let RulerLong        = 10
fileprivate let RulerShort       = 5
...
class DYRulerView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let shortLineY      = rect.size.height - CGFloat(RulerShort)
        let longLineY       = rect.size.height - CGFloat(RulerLong)
...
//                let attribute:Dictionary = [NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:UIColor.init(white: TextColorWhiteAlpha, alpha: 1.0)]
                let attribute:Dictionary = [NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:UIColor.black]
                let width = numStr.boundingRect(
                    with: CGSize(width: CGFloat.greatestFiniteMagnitude, height: CGFloat.greatestFiniteMagnitude),
                    options: NSStringDrawingOptions.usesLineFragmentOrigin,
                    attributes: attribute,context: nil).size.width
                numStr.draw(in: CGRect.init(x: startX+lineCenterX*CGFloat(i)-width/2, y: rect.size.height - 25 - 14, width: width, height: 14), withAttributes: attribute)
...
//        let attribute:Dictionary = [NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:UIColor.init(white: TextColorWhiteAlpha, alpha: 1.0)]
        let attribute:Dictionary = [NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:UIColor.blue]
...
class DYScrollRulerView: UIView {
    ...
    init(frame: CGRect,tminValue:Float,tmaxValue:Float,tstep:Float,tunit:String,tNum:Int) {
        ...
        lazyValueTF.backgroundColor = UIColor.blue
...
//        self.addSubview(self.lazyTriangle)
        ...
        addSubview(view)
        view.frame = CGRect(x: (self.bounds.size.width - 1.5) * 0.5, y: bounds.size.height - lazyValueTF.frame.maxY + 20, width: 1.5, height: 20)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.item == 0 {
            ...
            if rulerView == nil {
                ...
                rulerView!.backgroundColor   = UIColor.orange
                rulerView!.step              = step
...
extension DYScrollRulerView:UICollectionViewDelegate{
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        if !decelerate {
            self.setRealValueAndAnimated(realValue: (Float(scrollView.contentOffset.x) + Float(RulerGap) * 0.5)/Float(RulerGap), animated: true)
        }
    }
